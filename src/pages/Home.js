...
const Home = () => {
  ...
  const sendToChatGPT = async (userPrompt) => {
    ...
    try {
      ...
      if (data?.choices?.[0]?.message?.content) {
        ...
        setChatGPTResponse("No response from Hurudza AI please subscribe or contact us for free demo.");
        setIsModalOpen(true);
      }
    } catch (error) {
  ...
  return (
    <>
      <PreHeader />
      <Header />
      {/* Desktop-only hero with voice and text search */}
      <section className="hidden md:block relative my-10 bg-gradient-to-b from-emerald-50 via-white to-white">
        ...
        <div className="grid place-items-center">
          <div className="container max-w-5xl bg-white/80 backdrop-blur p-10 mt-6 text-center rounded-2xl shadow-xl border border-emerald-100">
            <p className="text-3xl md:text-4xl font-semibold text-emerald-700 tracking-tight">
              Welcome to Hurudza AI
            </p>
            <p className="text-sm md:text-base text-gray-600 mt-2">Search anything about agriculture on Hurudza AI</p>
            ...
            <div className="flex flex-col items-center space-y-3 my-6">
              <div className="w-full max-w-3xl flex flex-row space-x-2">
                <input
                  ...
                  placeholder="Ask anything (e.g., best fertilizer for tomatoes)"
            ...
            {/* Language Buttons */}
            <div className="flex flex-row flex-wrap gap-2 my-4 justify-center items-center">
